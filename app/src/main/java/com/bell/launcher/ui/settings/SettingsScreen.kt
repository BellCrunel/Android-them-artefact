...
fun SettingsScreen(
    ...
    Column(
        ...
        Column(
            ...
            RowItem(title = "Шпалери системи", subtitle = "Обрати з галереї", onClick = onChangeWallpaper)
            ...
            SwitchItem(
                title = "Показувати погоду",
                checked = settings.weatherEnabled,
                onChange = onWeatherEnabled,
            )
            SwitchItem(
                title = "Брати з геолокації",
                checked = settings.useLocation,
                onChange = onUseLocation,
            )
            RowItem(
                title = "Місто вручну",
...
private fun GestureItem(
    ...
    Box {
        Row(
            Modifier.fillMaxWidth().clickable { open = true }.padding(vertical = 12.dp),
            verticalAlignment = Alignment.CenterVertically,
        ) {
            Text(slot.title, Modifier.weight(1f), style = MaterialTheme.typography.bodyLarge)
            Text(
                current.title,
